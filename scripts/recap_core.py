...
def format_session_entry(
    ...
    if git_context.get("is_git"):
        if git_context.get("status_clean"):
            ...
            lines.append(
                f"{git_context['uncommitted_count']} file(s) with uncommitted changes."
            )
            lines.append("")
...
def format_restore_context(
    recent_sessions: str, git_context: dict
) -> str:
    ...
    for jf in journal_files:
        ...
        for match in re.finditer(
            ...
            branch_match = re.search(
                r"\*\*Branch\*\*:\s*(.+)", content[pos : pos + 200]
            )
            if branch_match:
